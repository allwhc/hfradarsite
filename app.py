...
# Get attendance records (admin - filter by site and/or date)
...
def adminGetAttendance():
    try:
        ...
        query = "SELECT id, staff_name, site_code, latitude, longitude, timestamp, distance_m FROM attendance WHERE 1=1"
        params = []
        if site_code:
            query += " AND site_code=?"
